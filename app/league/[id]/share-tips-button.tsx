...
export function ShareTipsButton({ leagueId, inviteUrl }: Props) {
  ...
  return (
    <div className="relative shrink-0">
      ...
      {/* Inline status messages below the button */}
      {status === "notReady" && (
        <p className="absolute top-full left-0 mt-1 text-[11px] text-muted-foreground whitespace-nowrap bg-background border border-border rounded px-2 py-1 shadow-sm z-10">
          Tippa klart alla matcher först
        </p>
      )}
      {status === "copied" && (
        <p className="absolute top-full left-0 mt-1 text-[11px] text-green-600 whitespace-nowrap bg-background border border-green-200 rounded px-2 py-1 shadow-sm z-10">
      ...
      {status === "error" && (
        <p className="absolute top-full left-0 mt-1 text-[11px] text-destructive whitespace-nowrap bg-background border border-destructive/20 rounded px-2 py-1 shadow-sm z-10">
